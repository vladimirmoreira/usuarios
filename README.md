...
| 4 | ✅ **Catálogos master (`TMP$USUARIO_PERMISOS_MASTER` / `TMP$USUARIO_MENU_MASTER`)** — RESUELTO (2026-07-10) | Se **crearon y sembraron** en `orgonita_master` (9 permisos + 19 menú, títulos ASCII para evitar mojibake en BD `NONE`), y se agregó su creación+seed a `migrarDDL()` (guardada por master configurado) para instalaciones nuevas. El catálogo ahora lee de la BD (sin el `-204`). Se mantiene el **fallback** hardcodeado en `catalogo.model.js` como respaldo. |
