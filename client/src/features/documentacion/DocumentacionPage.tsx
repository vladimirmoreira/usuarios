...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    bloques: [
      ...
      { t: 'sub', texto: 'Cascada profunda del legajo' },
      { t: 'p', texto: 'Si el usuario tiene mesero con legajo, antes de escribir GG_MESERO se garantizan sus dependencias en orden de clave foránea, replicándolas de central si faltan en el destino:' },
      { t: 'p', texto: 'PROFESION · PAIS · CIUDAD → DEPGEOGRAFICO · BARRIO → CIUDAD · RH_ESTUDIO → RH_PERSONA → RH_DPTO → RH_CARGO → GG_MESERO.' },
      { t: 'p', texto: 'Es best-effort por dependencia: si una no se puede resolver, se marca Bloqueado y no se aborta el resto. Los datos base con id 0 (centinela) deben existir en la sucursal como dato de instalación; no se replican por usuario.' },
      { t: 'sub', texto: 'Guardas de FK en conceptos' },
      { t: 'p', texto: 'En USUARIO_CONCEPTO se verifica el TIPOMOVIMIENTO (si falta, se omite ese concepto) y los FK opcionales (talonario, vendedor, persona, plan de venta, condición): si su destino no existe se anulan; el valor 0 se trata como "sin referencia".' },
    ],
  },
  {
